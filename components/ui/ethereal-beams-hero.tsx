...
// Either a real <button> (no href) or a real <a> (href present) — never a
// button-wrapping-an-anchor, which is invalid HTML and breaks navigation
// and screen-reader semantics.
type ButtonProps =
  | (ButtonOwnProps & { href?: undefined } & React.ButtonHTMLAttributes<HTMLButtonElement>)
  | (ButtonOwnProps & { href: string } & React.AnchorHTMLAttributes<HTMLAnchorElement>)
...
const Button = ({ variant = "default", size = "sm", className = "", children, ...props }: ButtonProps) => {
  ...
  const variants = {
    ...
    outline: "border border-white/30 bg-black/40 backdrop-blur-xl text-white hover:bg-black/55 hover:border-white/50",
    ghost: "border border-white/15 bg-black/30 backdrop-blur-xl text-white hover:text-white hover:bg-black/45",
  }
  ...
  if ("href" in props && props.href !== undefined) {
    const { href, ...anchorProps } = props as ButtonOwnProps & { href: string } & React.AnchorHTMLAttributes<HTMLAnchorElement>
    return (
...
/**
 * Ethereal Beams Hero — adapted for Arun Kumar Bathini's DevOps portfolio.
 *
 * The built-in navbar from the original template was removed: this site
 * already has a persistent `SiteHeader` (sticky, real nav links) rendered
 * above every page, so a second nav here would duplicate it. This component
 * now supplies only the animated beams backdrop + hero content, sized to
 * sit directly under that header.
 */
export default function EtherealBeamsHero() {
  const stats = impactMetrics.slice(0, 3)

  return (
    <div className="relative min-h-[calc(100vh-88px)] w-full overflow-hidden bg-black">
      {/* Beams Background */}
      ...
      <div className="relative z-10 flex min-h-[calc(100vh-88px)] items-center">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="mx-auto max-w-4xl text-center">
            {/* Badge */}
            <div className="mb-8 inline-flex items-center rounded-full bg-black/40 backdrop-blur-xl border border-white/15 px-4 py-2 text-sm text-white/90">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-12">
              <Button href="#work" size="lg" className="shadow-2xl shadow-white/25 font-semibold">
                View Work
                <ArrowRight className="ml-2 h-5 w-5" />
              </Button>
              <Button href={profile.resumeHref} download size="lg" variant="outline" className="font-semibold bg-transparent">
